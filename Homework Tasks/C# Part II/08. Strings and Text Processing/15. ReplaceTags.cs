...
class ReplaceTags
{
    static void Main()
    {
        ...
        string pattern = @"<\s*?a\s[^>]*\bhref\b\s*=\s*((['""](?<hyperlink>[^\""']*)[""'])|(?<hyperlink>\S*))>(?<htext>(.|\s)*?)<\s*/a\s*>";
        string output = Regex.Replace(input, pattern, m => "[URL=" + m.Groups["hyperlink"] + "]" + m.Groups["htext"] + "[/URL]");
